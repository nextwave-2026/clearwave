...
class ContinuousLoopTests(unittest.TestCase):
    ...
    def test_on_batch_runs_on_empty_polls_so_a_quiet_topic_still_sweeps(self) -> None:
        """`--detect-every` is a wall-clock interval, not a traffic interval."""
        calls: list[int] = []
        source = QuietSource()
        consumer.consume(
            self.connection,
            source,
            idle_polls=3,
            on_batch=lambda _progress: calls.append(1),
        )
        # Three empty polls plus the final flush after the loop.
...
class GroundTruthIsolationTests(unittest.TestCase):
    ...
    def test_the_service_is_the_daemon_not_a_bash_loop(self) -> None:
        block = self.services["detector"]
        self.assertNotIn("while true", block)
        self.assertNotIn("idle-polls", block)
        self.assertIn("detector/Dockerfile", block)
