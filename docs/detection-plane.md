...
The decisions behind everything below are recorded as [ADRs 0015 to 0021](adr/README.md). The
...
A drop qualifies only when all four floors hold together ([ADR 0015](adr/0015-detection-floors-not-a-single-threshold.md)):
...
Priced per payment, never per attempt ([ADR 0019](adr/0019-value-is-priced-per-payment.md)), and
labelled GMV at risk rather than platform revenue. Every response carries its assumptions.

### L5 - the severity

Four components, then a money ceiling ([ADR 0016](adr/0016-severity-is-bounded-by-money.md)):
...
([ADR 0017](adr/0017-localisation-descends-on-contrast.md)):
...
| Property | Mechanism |
|---|---|
| Replay equals live | every bucket, window and onset is computed from event time behind a watermark ([ADR 0018](adr/0018-event-time-bucketing.md)); the same log replayed produces an identical incident |
