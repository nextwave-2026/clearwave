...
**Leave it running. The store is pre-warmed.** `make stack-up` writes eight event-time hours of healthy merchant-b/adyen history into `state/clearwave.db` (`python3 -S scripts/prepare_history.py`, also `make stack-prepare`) before compose starts. That is seconds of generation, not an hour of waiting, and it is a clean start: last rehearsal's incident rows are dropped so they cannot promote a later band. Detection still compares the current window against that merchant's own trailing hour (`BASELINE_TRAILING_BUCKETS = 60` of `BUCKET_SECONDS = 60` in `detector/config.py`). Merchant-relative severity still needs `MERCHANT_NORMAL_MIN_HOURS = 6` and `MERCHANT_NORMAL_MIN_PAYMENTS = 200`; below those floors `merchant_normal_hourly_value_usd` and `severity_ceilings.merchant_relative` both read null. `make stack-status` reports those two floors separately. Do not pre-create an incident or a watch. The judge still presses the button; the detector still has to find the live deviation itself.

### Which path to use

| Path | Standing |
| --- | --- |
| **Pre-warmed compose stack** (`make stack-up`, browser on port 8082) | **The live product.** Detector, investigation and dashboard run themselves against `state/clearwave.db`. `stack-up` seeds healthy history first, so the trailing-hour baseline and the six-hour merchant-relative floor are warm when compose comes up. |
