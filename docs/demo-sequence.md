...
## Deterministic fallback - prepared watch, live escalation

Use this path when the main live path did not form its watch during pre-flight. It is a first-class, honest staged demonstration, not a fake screen or a prewritten incident.

### Captain setup

Run this one command immediately before handing the browser to the judges. It fully tears down and resets only the isolated fallback project, including its SQLite store and Kafka volume, then prepares the store and starts the stack:

```sh
.venv/bin/python -S scripts/fallback_demo.py
```

The fallback uses project `clearwave-fallback-demo`, store `state/fallback-demo/clearwave.db`, and dashboard **http://127.0.0.1:38082/**. It never touches the live stack on 8082/9092. The first run may build images; a repeat rehearsal can use `--no-build`. Do not run it early and leave it unattended: the detector starts after a short gate, so the prepared watch is stable while the judges arrive and press the live second stage.

The command does four real things before the browser opens:

1. replaces the store and writes eight event-time hours of healthy live-vocabulary history;
2. writes 240 additional canonical attempts for merchant-b / adyen in the PSE cohort, with a seeded developing approval rate;
3. runs the real detector, which must persist exactly one `watching` record and no incident; and
4. runs the real investigation against that watch before starting the dashboard, returning the record to `watching` with its C4 result.

The prepared attempts are simulated data generated by this repository, not a hardcoded outcome. The watch, its money, floors, query trail and investigation result are all written by the existing detector, store and investigation code. The detector service then starts automatically after its gate. Only merchant-b's real worker is started in this isolated stack; the judge's later **Collapse** press is sent through the normal Kafka control topic and produces the second stage live.

### Exact words for the captain

Say this before the judge operates the page:

> "I prepared a finite block of real simulated payment attempts before you arrived. The deviation is already under way, and ClearWave detected it itself from the attempts in its store; this warning is not a pre-rendered card and I did not insert a watch row. I am being explicit about the preparation: we are starting at the watch so we do not spend your four minutes waiting for a signal to accumulate. You will now press the live collapse control, and we will see whether the same stored record becomes an incident."

Do not say that the judge caused the watch, that the watch formed live during this session, or that the prepared data came from a real company. All names and traffic are simulated.

### Judge-operated sequence

**0. Watch already visible - about 30 seconds**

The judge clicks **Revenue impact**, then the prepared row in the **Watching** rail. The row shows the measured cohort, projected loss labelled **if this continues**, the trajectory and the floors that remain open. **Escalation** shows no page for the watch. The investigation has already run against this watch, but the primary watch rail exposes deterministic advice rather than a causal diagnosis; do not imply that the detailed C4 view is available before detection.

Say:

> "This is an early, merchant-relative warning. It compares the cohort with this merchant's recent behaviour and sibling cohorts. The incident floors are still open, so the watch is low and does not page. The preventive advice is visible now; the detailed causal result is not presented until detection, and uncertainty will remain explicit."

**1. Escalate the same record - about 90 seconds to the first detector sweep, then allow the normal investigation wait**

The judge clicks **Collapse** once. Do not click **Developing**: the developing stage is the prepared starting condition in this path. The status must report that the judge started the collapse in merchant-b's adyen traffic and must say `delivered: true` in the browser's result.

The board then measures the live collapse. When it crosses the floors, the prepared watch upgrades on the same incident id, the cohort can sharpen to `{merchant_id: merchant-b, provider: adyen, country: CO}`, and the queue shows stored revenue impact. Open **Incident detail**, **Evidence trail** and **Escalation**. Slack is a delivery claim only if the Escalation view records it; never infer delivery from the binding.

Say:

> "The warning was prepared from real traffic, but this escalation is live. Detection is deciding from the new event-time evidence, not from this button and not from the model. Point to the unchanged incident id: this is the same record moving from preventive watch to incident. The model can explain the evidence and recommend a human action, but it does not set severity or page a channel."

**2. Stop without overselling recovery**

End the fallback after the incident, investigation and Escalation views. The browser's **Clear** control stops the worker's injected effect, but this fallback does not claim automatic closure of an already diagnosed record: the current detector does not turn a diagnosed row into resolved merely because traffic recovers. If the judge presses **Clear**, say:

> "The control was delivered and the worker stopped the live effect. This prepared path does not claim that the diagnosed record has already closed; its recovery lifecycle is outside this fallback."

Keep the historical incident visible. Do not call the board recovered or hide the row.

### If the fallback still fails

- If the command refuses to persist one watch, or the browser opens without exactly one watch and zero active incidents, stop. Do not edit the database or call the row healthy; use the static evidence tour and label it precomputed.
- If the **Collapse** response is not delivered, say: "The live second-stage input did not reach Kafka, so I will not claim an escalation." Show the prepared watch and its deterministic advice only.
- If the incident id changes, say: "The detector produced a new record, so the same-record guarantee was not demonstrated on this run." Do not merge the records verbally.
- If the investigation is running or unavailable, show the deterministic incident and executed evidence. Say: "The causal narrative is still running or unavailable; I will not invent one." The escalation beat remains valid only if its stored state is visible; do not claim a causal diagnosis.
- If Clear is not delivered or the active row remains after its allowance, say so and end with the stored incident. Do not delete it or call the board recovered.

### What this fallback does not show

Compared with the full live path, this path does **not** show the judge causing the initial watch, the developing-deviation control, or watch formation from a healthy board during the four-minute judge window. It starts after a captain-prepared detector run, uses one isolated merchant worker and a short detector start gate, and does not demonstrate two simultaneous incidents, the phone channel, hidden-truth scoring, or an unrehearsed input. It still demonstrates genuine merchant-relative measurement, a watch with preventive advice and no page, live collapse escalation on the same record when the run reaches it, and evidence-bound investigation. It does **not** demonstrate the final clear/closure lifecycle, because the current product does not close an already diagnosed row automatically after recovery; do not claim that it does. If any live beat fails, present only what the store actually shows.
