...
themselves. This module is the operator surface for the compose `detector`
service. It is deliberately thin.
...
`cli._periodic_sweeper`. The only things a service needs on top of that loop
are: empty polls are never terminal (`idle_polls=0`), a signal can ask it to
stop without interrupting a batch (`should_stop`), and a quiet minute still
sweeps because `on_batch` runs after empty polls too, not only after a write.
